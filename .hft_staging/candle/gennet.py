...
def main():
    ...
    # addressed RAM: depth slots x lanes; slot-major so addr = base + slot*L + ofs.
    ram_cells = []
    if ram:
        rdepth = ram["depth"]; rlanes = ram["lanes"]
        for s in range(rdepth):
            for li, ln in enumerate(rlanes):
                nm = f"{ram['name']}_{s}_{ln['name']}"
                ram_cells.append(nm); order.append(nm)
    ...
    if ram:
        consumed.add(ram["wr_index"]); consumed.add(ram["wr_fire"])
        for ln in ram["lanes"]:
            consumed.add(ln["source"])
    ...
    # ---- addressed RAM write (single-port): slot[wr_index] <- lanes on wr_fire.
    # ADDRESS arithmetic only (base + slot*stride + ofs) — not a data-path op.
    if ram:
        rlanes = ram["lanes"]; L = len(rlanes)
        base0 = f"{pfx}_{ram['name']}_0_{rlanes[0]['name']}"
        out.append("    { /* addressed RAM write port */")
        out.append(f"        const word_t _ram_wa = ({lc(ram['wr_index'])}) & {ram['depth'] - 1}ULL;")
        out.append(f"        const word_t _ram_we = {lc(ram['wr_fire'])};")
        for li, ln in enumerate(rlanes):
            idx = f"{base0} + _ram_wa*{L}ULL + {li}ULL"
            out.append(f"        r[{idx}] = cell_dff(r[{idx}], {lc(ln['source'])}, _ram_we);")
        out.append("    }")
        out.append("")
