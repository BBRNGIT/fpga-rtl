...
- `FIFO_RX_TAI` — TAI timestamp from ingress
  - Source: `fifo_rx` module
  - Type: `u64`
...
```
TPO_BAR_START_TAI            — 64-bit flip-flop
                            Comment: TAI timestamp when bar opened
                            Type: u64
```

```
TPO_OPEN_SET            — 64-bit flip-flop
                            Comment: First price level seen in bar (bar open)
                            Type: u64
```

```
TPO_PREV_TAI            — 64-bit flip-flop
                            Comment: Last TAI timestamp processed
                            Type: u64
```

```
TPO_PREV_TRADE_CNT            — 64-bit flip-flop
                            Comment: Last trade count seen (for delta calculation)
                            Type: u64
```

```
TPO_BAR_MIN_PRICE            — 64-bit flip-flop
                            Comment: Minimum price touched in bar
                            Type: u64
```

```
TPO_BAR_MAX_PRICE            — 64-bit flip-flop
                            Comment: Maximum price touched in bar
                            Type: u64
```

```
TPO_BAR_POC_PRICE            — 64-bit flip-flop
                            Comment: Price level with most time exposure (running bar)
                            Type: u64
```

```
TPO_BAR_POC_TOTAL            — 64-bit flip-flop
                            Comment: Total time at bar POC level
                            Type: u64
```

```
TPO_BAR_POC_BID_TIME            — 64-bit flip-flop
                            Comment: Bid-side time at bar POC level
                            Type: u64
```

```
TPO_BAR_POC_ASK_TIME            — 64-bit flip-flop
                            Comment: Ask-side time at bar POC level
                            Type: u64
```

```
TPO_BAR_TIME_DELTA            — 64-bit flip-flop
                            Comment: Ask time - bid time at bar POC level
                            Type: u64
```

```
TPO_BAR_SEQ            — 64-bit flip-flop
                            Comment: Bar sequence number
                            Type: u64
```

```
TPO_BAR_PARITY            — 64-bit flip-flop
                            Comment: Virtual-reset parity tag (bit 63 of per-level entries)
                            Type: u64
```

```
TPO_BAR_CLOSED            — 64-bit flip-flop
                            Comment: Bar close flag (1 = closed)
                            Type: u64
```

```
TPO_COMP_POC_PRICE            — 64-bit flip-flop
                            Comment: Completed bar POC price
                            Type: u64
```

```
TPO_COMP_POC_BID_TIME            — 64-bit flip-flop
                            Comment: Completed bar POC bid time
                            Type: u64
```

```
TPO_COMP_POC_ASK_TIME            — 64-bit flip-flop
                            Comment: Completed bar POC ask time
                            Type: u64
```

```
TPO_COMP_TIME_DELTA            — 64-bit flip-flop
                            Comment: Completed bar time delta
                            Type: u64
```

```
TPO_COMP_BAR_SEQ            — 64-bit flip-flop
                            Comment: Completed bar sequence
                            Type: u64
```

```
TPO_COMP_BAR_TAI            — 64-bit flip-flop
                            Comment: Completed bar close TAI timestamp
                            Type: u64
```

```
TPO_COMP_MIN_PRICE            — 64-bit flip-flop
                            Comment: Completed bar minimum price
                            Type: u64
```

```
TPO_COMP_MAX_PRICE            — 64-bit flip-flop
                            Comment: Completed bar maximum price
                            Type: u64
```
...
```
TPO_BID_TIME[16384]    — 64-bit flip-flop array
                                        Depth: 16384 entries
                                        Index: PRICE_IDX
                                        Comment: Cumulative bid-side time per price level (parity-tagged)
                                        Type: u64
```

```
TPO_ASK_TIME[16384]    — 64-bit flip-flop array
                                        Depth: 16384 entries
                                        Index: PRICE_IDX
                                        Comment: Cumulative ask-side time per price level (parity-tagged)
                                        Type: u64
```
...
```
TPO_POC_OUT            — 64-bit relay output
                            Source: TPO_COMP_POC_PRICE
                            Comment: Completed POC price relay output
                            Type: u64
```

```
TPO_POC_BID_OUT            — 64-bit relay output
                            Source: TPO_COMP_POC_BID_TIME
                            Comment: Completed POC bid time relay output
                            Type: u64
```

```
TPO_POC_ASK_OUT            — 64-bit relay output
                            Source: TPO_COMP_POC_ASK_TIME
                            Comment: Completed POC ask time relay output
                            Type: u64
```

```
TPO_TIME_DELTA_OUT            — 64-bit relay output
                            Source: TPO_COMP_TIME_DELTA
                            Comment: Completed time delta relay output
                            Type: u64
```
...
Register Offsets (exact layout TBD per backplane allocation):
  ...
  0x0000 — TPO_BAR_START_TAI

  0x0008 — TPO_OPEN_SET

  0x0010 — TPO_PREV_TAI

  0x0018 — TPO_PREV_TRADE_CNT

  0x0020 — TPO_BAR_MIN_PRICE

  0x0028 — TPO_BAR_MAX_PRICE

  0x0030 — TPO_BAR_POC_PRICE

  0x0038 — TPO_BAR_POC_TOTAL

  0x0040 — TPO_BAR_POC_BID_TIME

  0x0048 — TPO_BAR_POC_ASK_TIME

  0x0050 — TPO_BAR_TIME_DELTA

  0x0058 — TPO_BAR_SEQ

  0x0060 — TPO_BAR_PARITY

  0x0068 — TPO_BAR_CLOSED

  0x0070 — TPO_COMP_POC_PRICE

  0x0078 — TPO_COMP_POC_BID_TIME

  0x0080 — TPO_COMP_POC_ASK_TIME

  0x0088 — TPO_COMP_TIME_DELTA

  0x0090 — TPO_COMP_BAR_SEQ

  0x0098 — TPO_COMP_BAR_TAI

  0x00A0 — TPO_COMP_MIN_PRICE

  0x00A8 — TPO_COMP_MAX_PRICE
  ...
  0x0108 — TPO_POC_BID_OUT

  0x0110 — TPO_POC_ASK_OUT

  0x0118 — TPO_TIME_DELTA_OUT
  ...
  0x004000 onwards — TPO_BID_TIME[16384]

  0x014000 onwards — TPO_ASK_TIME[16384]
...
Downstream (indicator/strategy):
  ...
  TPO_POC_OUT → (relay lane)

  TPO_POC_BID_OUT → (relay lane)

  TPO_POC_ASK_OUT → (relay lane)

  TPO_TIME_DELTA_OUT → (relay lane)
