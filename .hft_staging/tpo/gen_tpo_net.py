#!/usr/bin/env python3
"""gen_tpo_net.py — tpo's netlist emitter (thin wrapper).

The real emitter is the GENERIC meta-tool ../gen_module_net.py. This wrapper runs
it on tpo_logic.yaml so the per-component flow
(`python3 gen_tpo_net.py > tpo.net.json`) is preserved and clean-room
reproducible. The netlist is a TOOL OUTPUT — never hand-edited.

    python3 gen_tpo_net.py > tpo.net.json
"""
import os
import runpy
import sys
...
LOGIC = os.path.join(HERE, "tpo_logic.yaml")
